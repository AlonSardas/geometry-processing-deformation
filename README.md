...
shape $\widetilde{\mathbf{x}}$, then we could optimize for the best possible shape $\mathbf{x}$ by minimizing $E$:
...
$$
\mathop{\text{min}}\_\mathbf{d} \int\_{\Omega} \lVert  {\nabla}\mathbf{d} \rVert \_F^2 \ dA \quad \text{ subject to } \mathbf{d}\_i =
